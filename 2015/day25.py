...
def compute_next_code(current_code: int) -> int:
    """
    Generate the next code based on the current code

    Args:
        current_code (int): The current code to generate the next code from

    Returns:
        int: The next code in the sequence
    """
    return (current_code * 252533) % 33554393

def get_code_index(row: int, column: int) -> int:
    """
    Calculate the index of the code based on the row and column
    ...
    Returns:
        int: The index of the code in the sequence
    """
    return (row + column - 1) * (row + column) // 2 - (row - 1)
    # return sum(range(row + column - 1)) + column

def find_code_at_position(row: int, column: int) -> int:
    """
    Find the code at a specific row and column
    ...
    Returns:
        int: The code at the specified row and column
    """
